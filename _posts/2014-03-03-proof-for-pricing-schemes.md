...
###Proposition 1: 
**If the price announced in Subnetwork 1 is $p_1$, the price announced in Subnetwork 2 $p_2$ should be in $[u(s_1), p_{max}]$, where $s_1$ is the strongest Qos requirement in Subnetwork 1 which can be calculated by solving  , and $p_max$ is determined by the strongest Qos requirement may exist in the network.**
